...
parser.add_argument('--data_sample', type=int, default=100, help='Number of data samples to use for training')
parser.add_argument('--output_dir', type=str, default='.', help='Base directory to save the final model and checkpoints')
parser.add_argument('--wandb_project', type=str, default='es', help='Weights & Biases project name')
parser.add_argument('--wandb_run_name', type=str, default=None, help='Weights & Biases run name (optional)')
parser.add_argument('--wandb_entity', type=str, default=None, help='Weights & Biases entity/team name (optional)')
parser.add_argument('--eval_dataset_size', type=int, default=1, help='Number of samples for final evaluation (default: all)')
...
def extract_assistant_response(gen_text, model_family):
    """Extract assistant response based on model family"""
    if model_family == "qwen":
        if "assistant:" in gen_text:
            return gen_text.split("assistant:")[-1].strip()
    elif model_family == "llama":
        # Llama 3 format
        if "<|start_header_id|>assistant<|end_header_id|>" in gen_text:
            return gen_text.split("<|start_header_id|>assistant<|end_header_id|>")[-1].strip()
        # Llama 2 format
        elif "[/INST]" in gen_text:
...
def main():
    ...
    for item in data_json:
        ...
        target = item['target']
        full_dataset.append((context, target))

    # Use a subset of the dataset for training
    dataset = full_dataset[:args.data_sample]
    eval_dataset = full_dataset[args.data_sample:args.data_sample+args.eval_dataset_size]
    ...
    if accelerator.is_main_process:
        try:
            wandb.init(entity="anindya-umass-umass-amherst", project=f"{args.wandb_project}", name=f"{args.model_name.replace('/', '_')}_seed{initial_seed}_pop{POPULATION_SIZE}")
            wandb.config.update({
                "sigma": SIGMA,
                "alpha": ALPHA,
    ...
    # Llama models may not have a default pad token
    if model_family == "llama" and tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
        tokenizer.pad_token_id = tokenizer.eos_token_id
        if accelerator.is_main_process:
            print("Llama model: setting pad_token = eos_token")
    ...
    if accelerator.is_main_process:
        ...
        if os.path.exists(seed_trace_path):
            with open(seed_trace_path, "r") as f:
                seed_trace = json.load(f)
            # Append new execution entry to history
            ...
            # First run — initialize execution history
            seed_trace["header"]["execution_history"].append({
                "job_id": os.environ.get("SLURM_JOB_ID", "unknown"),
                "start_timestamp": time.strftime("%Y-%m-%dT%H:%M:%S"),
                "iterations_completed": 0
    ...
    for iteration in range(NUM_ITERATIONS):
        ...
        if accelerator.is_main_process:
            for seed_idx, seed in local_seeds:
                ...
                if (seed_val in seed_trace["data"] and
                    iter_key in seed_trace["data"][seed_val] and
                    "reward" in seed_trace["data"][seed_val][iter_key]):
                    cached_rewards[seed_idx] = seed_trace["data"][seed_val][iter_key]["reward"]
                    if args.verbose:
                        print(f"Skipping seed {seed_val} (idx {seed_idx}), reward already cached")
                else:
                    seeds_to_process.append((seed_idx, seed))
        else:
            seeds_to_process = local_seeds
        
        local_rewards = []

        # Add cached rewards directly
        for seed_idx, reward in cached_rewards.items():
            local_rewards.append((seed_idx, reward))

        # Only process uncached seeds
        if len(seeds_to_process) > 0:
        ...
        if accelerator.is_main_process:
            ...
            for seed_idx in range(POPULATION_SIZE):
                seed_val = str(seeds[seed_idx])
                iter_key = str(iteration + 1)
                raw_reward = float(rewards[seed_idx])
                norm_reward = float(rewards_normalized[seed_idx])
                if seed_val not in seed_trace["data"]:
                    seed_trace["data"][seed_val] = {}
                if iter_key not in seed_trace["data"][seed_val]:
                    seed_trace["data"][seed_val][iter_key] = {
                        "seed_idx": seed_idx,
                        "reward": raw_reward,
            ...
            # Update iterations_completed in latest execution entry
        ...
        # Evaluate on eval dataset every 10 iterations
        eval_mean_reward = None
        if (iteration + 1) % 10 == 0 and len(eval_dataset) > 0:
            if accelerator.is_main_process:
                eval_input_texts = [inp for inp, _ in eval_dataset]
                eval_target_texts = [tgt for _, tgt in eval_dataset]
                eval_rewards = evaluate_model(
                    model_list[0], tokenizer,
                    eval_input_texts, eval_target_texts,
                    accelerator, verbose=args.verbose, return_text=False
                )
                eval_mean_reward = sum(eval_rewards) / len(eval_rewards)
                print(f"  [Eval] Iteration {iteration + 1}: Eval Mean Reward = {eval_mean_reward:.4f}")
        ...
        if accelerator.is_main_process:
            ...
            # Record metrics to wandb and local history
            ...
            try:
                ...
                if eval_mean_reward is not None:
                    log_dict["eval_mean_reward"] = eval_mean_reward
                wandb.log(log_dict)
            except Exception:
                pass
